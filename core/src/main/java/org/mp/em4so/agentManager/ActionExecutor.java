...
public class ActionExecutor
{
	...
	/** The log. */
	private final Logger LOG = LoggerFactory.getLogger(ActionExecutor.class.getName());
	...
	public Action executeKnownAction(SmartObjectAgManager somanager, Activity activity, Action action)
			throws UnachievableGoalException
	{
		...
		try
		{
					...
					&& foundService.getExecutionInstance().getHost().getUrls() != null)
			{
				...
				LOG.trace(this.somanager.getId() + ": Found service:{} to execute w role {} on: {} result: {}",
						foundService.getName(), activity.getRole().getId(),
						foundService.getExecutionInstance().getHost().getUrls(), action.getResult());
				foundService.setExecutingRoleId(activity.getRole().getId());
				ServiceAssembler.executeService(foundService, loadArgValues(action, activity).getArgValues(),
	...
	public void executeAction(SmartObjectAgManager somanager, Action action, Activity activity)
	{
		...
		try
		{
			LOG.info(this.somanager.getId() + ": Starting execution of SERVICE:" + action.getService().getId() + " - "
					+ action.getService().getName());
			foundService = action.getService();
			foundService.setExecutionInstance(ServiceAssembler.populateExecutionInstance(action.getService()));
			if (foundService != null && foundService.getExecutionInstance() != null
					&& foundService.getExecutionInstance().getHost() != null
					&& foundService.getExecutionInstance().getHost().getUrls() != null)
			{

				LOG.trace(this.somanager.getId() + ": Found service:" + foundService.getName() + " to execute on: "
						+ foundService.getExecutionInstance().getHost().getUrls() + " result:" + action.getResult());
				foundService.setExecutingRoleId(activity.getRole().getId());
				ServiceAssembler.executeService(foundService, loadArgValues(action, activity).getArgValues(),
						loadResultElement(activity, action));
	...
	/** Look for result element data and load it, getting it ready for action
	 * execution.
	 *
	 * @param activity the activity
	 * @param action the action
	...
	public String loadResultElement(Activity activity, Action action) throws UnachievableGoalException
	{
		...
		if (action.getResult() != null && activity.getOutput() != null && !activity.getOutput().isEmpty())
		{
			...
			if (element != null && element.getName() != null && element.getScope() != null
					&& element.getAttributeName() != null && element.getKind() != null)
			{
				if (element.getName().contains("."))
				{
					indexI = Integer.parseInt(element.getName().substring(2, element.getName().indexOf("."))) - 1;
					attribute = element.getName().substring(element.getName().indexOf(".") + 1,
			...
			else
			{
				throw new UnachievableGoalException(
						"Invalid output specification " + action.getResult() + "  for activity " + activity.getId());
			}
	...
	/** Load actual arguments for actions from knowledge base according to
	 * specified in the activity inputs and outputs.
	 *
	 * @param action the action
	 * @param activity the activity
	 * @return the action */
	public Action loadArgValues(Action action, Activity activity)
	{
		...
		if (inputKnowledge != null)
			LOG.trace(" There are {} elements as input knowledge", inputKnowledge.size());
		if (outputKnowledge != null)
			LOG.trace(" There are {} elements as output knowledge", outputKnowledge.size());

		if (action != null && action.getArgValues() != null)
		{ // action has no arguments

			it = action.getArgValues().keySet().iterator();
			newArgValues = new Hashtable<String, Object>();

			if (action.getInputElements() == null)
				action.setInputElements(new LinkedList<Element>());
			...
			while (it.hasNext())
			{
				...
				if (value.startsWith("%"))
				{
					...
					newArgValues = getKnowledgeItemForService(knowledgeList, value, action, key, outputKnowledge,
							newArgValues);
				}
				else
	...
	public Hashtable<String, Object> getKnowledgeItemForService(List<Element> knowledgeList, String elementId,
			Action action, String key, List<Element> outputKnowledge, Hashtable<String, Object> newArgValues)
	{
		int index;
		String attribute = null;
		Element element = null;
		...
		if (elementId.contains("."))
		{
			...
			if (element.getKind() != null && !element.getKind().equals("message"))
			{
				...
				LOG.trace("Attributes is not NULL ->{}<- and attribute is not NULL {} ", element.getValue(), attribute);
				if (element.getValue() != null)
				{
					newArgValues.put(key, element.getValue());
					LOG.trace("-é-é-element.get... {}", element.getValue());
				}
				else if (element.getAttributes() != null || element.getAttributes().isEmpty())
				{
					LOG.trace("Atributes : " + element.getAttributes());
					LOG.trace("Attribute : " + attribute);
					newArgValues.put(key, element.getAttributes().get(attribute));
					LOG.trace("-é-é-element.get... {}", element.getAttributes().get(attribute));
				}
				else
				{
					LOG.error("-é-é-ERROR : value for " + key + " not found");
				}
			}
	...
	public boolean executeKnownActivity(Goal goal, Activity activity) throws UnachievableGoalException
	{
		...
		if (activity.getActions() != null && actions.size() > 0)
		{
			...
			{
				LOG.trace("Ready to process action Id is:{}, status: {} and Service name is {}", action.getId(),
						action.getStatus(), action.getService().getName());
	...
	{
		LOG.trace("{}: Starting update status of remote activity: {}. Status found is: {}  ", somanager.getId(),
				activity.getId(), somanager.getRemoteActivitiesStatus().get(activity.getId()));
	...
	public void notifyActivityDone(Role roleToNotify, Activity activity)
	{
		...
		if (host != null)
		{
			...
			LOG.trace("{}: Role {} notified of activity {} done.", somanager.getId(), roleToNotify.getId(),
					doneActivity.getId());
		}
		else
		{
			LOG.info("WARNING: Not available hosts for role {} to notify of activity {} done.", roleToNotify,
					activity.getId());
		}

	}

	/** Check status of the action and decides what action processing is
	 * required (execute, wait, error).
	 *
	 * @param goal
	 * @param activity the activity
	 * @param action the action
	...
	public boolean processAction(Goal goal, Activity activity, Action action) throws UnachievableGoalException
	{
		...
		LOG.trace("{} : (1) Start executing action: {} with status: ->{} / {} <- and service: id: ->{}<-, name: ->{}<-",
				somanager.getId(), action.getId(), action.getStatus(),
				somanager.getActionsStatus().get(activity.getId() + "." + activity.getSeq() + "." + action.getId()),
				action.getService().getId(), action.getService().getName());
		...
		LOG.trace("{} : (2) Processing by status action: {} with status: {} and {}", somanager.getId(),
				action.getService().getName(), action.getStatus(),
				somanager.getActionsStatus().get(activity.getSeq() + "." + action.getId()));
		...
		switch (action.getStatus())
		{
			case "pending":
			{
				...
				{
					prerequisite = false;
					LOG.trace("{}: (3) Prerequisites of action {} are : ", somanager.getId(),
							action.getService().getName());
					for (String pre : prereq)
					{
						prer = somanager.getActionsStatus().get(activity.getSeq() + "." + pre).getStatus();
						LOG.trace("{}: (4) - {} Prerequisite {} w status: {}", somanager.getId(), pre,
								somanager.getActionsStatus().get(activity.getSeq() + "." + pre), prer);
						if (prer != null && prer.equals("done"))
						{
							prerequisite = true;
						}
				...
				LOG.trace("{}: (5) Any pending prerequisite for action {}? {} ", somanager.getId(),
						action.getService().getName(), prerequisite);

				if (!prerequisite)
					action = executeKnownAction(somanager, activity, action);
			}
				break;
			case "onprogress":
			{
	...
	public void updateActivityStatus(Activity activity)
	{
		...
		LOG.trace("Updating status of activity {} to: {}", activity.getId(),
				somanager.getRemoteActivitiesStatus().get(activity.getId()));
	}
